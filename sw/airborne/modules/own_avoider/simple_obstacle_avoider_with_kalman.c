...
float plant_detect_threshold = 0.1f;
...
void simple_obstacle_avoider_periodic(void)
{
  ...
  dbg_counter++;
  if (dbg_counter % 10 == 0) {
    PRINT("raw orange=(%.3f %.3f %.3f) filt orange=(%.3f %.3f %.3f)\n",
          orange_raw.left_fraction, orange_raw.middle_fraction, orange_raw.right_fraction,
          orange_filtered.left_fraction, orange_filtered.middle_fraction, orange_filtered.right_fraction);
  }
