...
bool is_out_of_bounds = false; //added by Alex
int out_of_bounds_counter = 0;

float orange_detect_threshold = 0.02f;
float floor_detect_threshold = 0.05f;
float plant_detect_threshold = 0.05f;
float out_of_bounds_floor_threshold = 0.02f; //added by Alex
...
float base_forward_speed = 0.99f; //used to be 0.25f
float slow_forward_speed = 0.75f; //used to be 0.18f
float very_slow_forward_speed = 0.50f; //used to be 0.12f
float small_yaw_rate = 0.50f; //used to be 0.12f
float medium_yaw_rate = 0.69f; //used to be 0.18f
float search_yaw_rate = 0.25f; //used to be 0.10f
...
static struct kalman_1d kf_green_floor;
static struct kalman_1d kf_green_plant;
...
void temporal_filter(void){
  ...
  green_filtered.floor_fraction = kalman_update(&kf_green_floor, green_raw.floor_fraction);
  green_filtered.plant_fraction = kalman_update(&kf_green_plant, green_raw.plant_fraction);

  update_detection_flags(&orange_filtered, &green_filtered);
}
...
enum action decide_action(const struct orange_info *orange, const struct green_info *green, int confidence){
  ...
  // --- OUT OF BOUNDS DETECTION WITH MEMORY ---
  if (green != 0) {

    // detect edge
    if (green->floor_fraction < 0.02f) {
    ...
    // clamp
    if (out_of_bounds_counter < 0) out_of_bounds_counter = 0;
    if (out_of_bounds_counter > 10) out_of_bounds_counter = 10;

    // activate only if persistent
    if (out_of_bounds_counter > 3) {
      is_out_of_bounds = true;
    }

    // deactivate only if clearly safe
    if (out_of_bounds_counter == 0 && green->floor_fraction > 0.08f) {
      is_out_of_bounds = false;
    }

    // if out of bounds → force turn
    if (is_out_of_bounds) {
...
struct command action_to_command(enum action action, const struct orange_info *orange, const struct green_info *green){
  ...
  if (green != 0 && green->floor_fraction < 0.10f) {
    out_of_bounds = true;
  }
  ...
  switch (action) {
    case FORWARD:
      ...
      if (green != 0 && green->floor_fraction < 0.08f) {
        command.forward_speed = 0.15f;  // slow near edge
      } else {
        command.forward_speed = 0.30f;  // faster than before
      }
    ...
    case LEFT:
      if (out_of_bounds) {
        command.forward_speed = 0.0f; //used to be 0.8f
      } else if (middle >= middle_strong_threshold) {
        command.forward_speed = 0.5f;
      } 
      else {
        command.forward_speed = slow_forward_speed;	//ver_slow -> slo_forward
      }
    ...
    case RIGHT:
      if (out_of_bounds) {
        ...
        command.forward_speed = slow_forward_speed;
      }
...
void simple_obstacle_avoider_periodic(void)
{
  ...
  temporal_filter();
  update_confidence(&orange_filtered, &green_filtered);

  last_action = decide_action(&orange_filtered, &green_filtered, obstacle_confidence);
  last_command = action_to_command(last_action, &orange_filtered, &green_filtered);
  ...
  PRINT("action=%s conf=%d orange=(%.3f %.3f %.3f) green=(%.3f %.3f) cmd=(%.3f %.3f)\n",
        ...
        green_filtered.floor_fraction,
        green_filtered.plant_fraction,
