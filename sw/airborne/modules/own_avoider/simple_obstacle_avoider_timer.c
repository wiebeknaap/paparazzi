...
float base_forward_speed = 0.35f; //used to be 0.25f
float slow_forward_speed = 0.25f; //used to be 0.18f
float very_slow_forward_speed = 0.15f; //used to be 0.12f
float small_yaw_rate = 0.50f; //used to be 0.12f
float medium_yaw_rate = 1.0f; //used to be 0.18f
float search_yaw_rate = 0.6f; //used to be 0.10f
...
enum action decide_action(const struct orange_info *orange, const struct green_info *green, int confidence){
  ...
  // If currently turning → keep turning
  if (turn_timer > 0) {
    turn_timer--;
    return last_action;
  }
  ...
  switch (action) {
    ...
    case LEFT:
      if (out_of_bounds) {
        ...
        command.forward_speed = 0.0f;	//ver_slow -> slo_forward
      }
